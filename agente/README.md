# Agente 10.0.3.8 — SQL Backup Checker (Fase 4)

Agente **independiente** del backend (se ejecutará en el servidor 10.0.3.8 vía
Task Scheduler). Valida los respaldos SQL diarios contra el catálogo del sistema
y reporta el resultado; el backend decide las incidencias (§26).
...
1. Lee su configuración del backend  GET /api/v1/ingesta/configuracion (X-Agent-Key)
   → bases SQL, ruta origen, horarios esperados por día (§35: nada quemado).
2. Para cada base SQL valida la carpeta origen:
...
├── scripts/
│   └── simular_respaldos.py # Genera .bak falsos para pruebas locales
